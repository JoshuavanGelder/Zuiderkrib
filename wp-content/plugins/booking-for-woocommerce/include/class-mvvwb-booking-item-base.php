    ...
    public function checkRules()
    {
        ...
        foreach ($rules as $rule) {
            ...
            if ($rule['type'] === 'dateRange') {
                ...
                if ('yes' === $rule['bookable'] || 'holiday' === $rule['bookable']) {
...
//                        });
//                    }
                    $unAvailableDays = mvvwb_popDates($unAvailableDays, $start, $end);

                    if ('holiday' === $rule['bookable']) {
                        for ($s = $start; $s <= ($end); $s = ($s + 86400)) { // add seconds
                            $holidays[] = $s;
                        }
                    } else {
                        $holidays = mvvwb_popDates($holidays, $start, $end);
...
//                    }
                ...
                if ('yes' === $rule['bookable'] || 'holiday' === $rule['bookable']) {
                    ...
                    if ('holiday' === $rule['bookable']) {
                        $weekHolidays = array_unique(array_merge($weekHolidays, $days));
                    } else {
                        if (count($weekHolidays)) {
                            $weekHolidays = array_filter($weekHolidays, function ($va) use ($days) {
                                if (in_array($va, $days)) {
                                    return false;
                                }
                                return true;
                ...
                if ('yes' === $rule['bookable'] || 'holiday' === $rule['bookable']) {
//                    if (count($unAvailableHours)) {
//                        $unAvailableHours = array_filter($unAvailableHours, function ($va) use ($start, $end) {
//                            if ($va <= $end && $va >= $start) {
//                                return false;
//                            }
//                            return true;
//                        });
//                    }
                    $unAvailableHours = mvvwb_popDates($unAvailableHours, $start, $end);
                    if ('holiday' === $rule['bookable']) {
                        $perBlockDuration = $this->config['general']['blockDuration'];
                        $unitInMinutes = $this->getUnit() === 'hours' ? $perBlockDuration * 60 : $perBlockDuration;
                        for ($s = $start; $s <= ($end); $s = ($s + $unitInMinutes)) { // add seconds
                            $holidayHours[] = $s;
                        }
                    } else {
                        $holidayHours = mvvwb_popDates($holidayHours, $start, $end);
                    }
                } else {
                    $perBlockDuration = $this->config['general']['blockDuration'];
                    $unitInMinutes = $this->getUnit() === 'hours' ? $perBlockDuration * 60 : $perBlockDuration;
                ...
                $d = clone $this->bookingData['start'];
                $dayNumber = $d->format('N');
                if (in_array($dayNumber, $rule['days'])) {
                    $start = ($rule['timeRange']['start']) * 1;
                    $end = ($rule['timeRange']['end']) * 1;
                    if (!($start && $end && $start <= $end)) {
                        continue;
                    }
                    if ('yes' === $rule['bookable'] || 'holiday' === $rule['bookable']) {
        ...
        foreach ($holidays as $day) {
            ...
            $_start = $this->bookingData['start']->getTimestamp();
            if ($_start >= $day && $_start < ($day + $this->getPerBlockMinutes() * 60)) {
                return false;
            }
        }
        ...
        if ($weekHolidays && count($weekHolidays)) {

            $weekHolidays = array_map(function ($d) {
                if ($d == 0) {
                    return 7; //
                }
                return $d;
            }, $weekHolidays);
            foreach ($weekHolidays as $day) {
                if ($this->bookingData['start']->format("N") == $day) {
                    return false;
                }
            }
        }
        if ($this->isUnitType(['hours', 'minutes'])) {
            foreach ($unAvailableHours as $hr) {
                $d = clone $this->bookingData['start'];
                $d->setTime(0, $hr, 0);
            ...
            foreach ($holidayHours as $hr) {
                $d = clone $this->bookingData['start'];// to get date of selection
                $d->setTime(0, $hr, 0);
                $_d = $d->getTimestamp();
                $_start = $this->bookingData['start']->getTimestamp();

                if ($_start >= $_d && $_start < ($_d + $this->getPerBlockMinutes() * 60)) {
                    return false;
                }
            }
        }
